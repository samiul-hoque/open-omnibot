// Smoke-test runner for circle_0_5m_strafe.
//
// Arms the trajectory, starts it, and streams the runner's own events
// (preflight, state changes, ticks, completion). Auto-submits the
// ground-truth form at completion using the runner's last-waypoint
// auto-fill so the runner cleans up to idle without the operator
// having to touch the browser.
//
// Inspect afterwards: evaluation/snapshots/circle_0_5m_strafe/<runId>/*.png +
// the run's meta.json under server/logs/.
import WebSocket from 'ws';

const URL = 'ws://localhost:3000/ws';
const TRAJECTORY = 'circle_0_5m_strafe';
const SPEED = 0.10;

// Motion budget: circumference 2π·0.5 = 3.14 m at 0.10 m/s ≈ 31.4 s.
// Plus 4 × (settle 300 ms + snapshot lead 2500 ms) ≈ 11.2 s pause
// overhead ≈ 43 s total. Allow headroom for GT submit + slop.
...
ws.on('open', () => {
    ...
    send({
        ...
        speed: SPEED,
        rep: 1,
        operatorNotes: 'smoke test — circle_0_5m_strafe from (0.00, -0.40) +Y',
    });
});
