// Smoke-test runner for yaw_roundtrip.
//
// Four consecutive in-place 90° yaws: CCW → CW → CW → CCW. Debug
// trajectory for yaw-accuracy investigation; not used for the thesis.
//
// Inspect afterwards: evaluation/snapshots/yaw_roundtrip/<runId>/*.png
// + the run's meta.json under server/logs/.
import WebSocket from 'ws';

const URL = 'ws://localhost:3000/ws';
const TRAJECTORY = 'yaw_roundtrip';
const SPEED = 0.10;                // Unused by this trajectory; kept for runner wiring

// Motion budget: 4 × 90° at 0.5 rad/s = 4π s ≈ 12.6 s + 5 × (settle
// 300 ms + snapshot lead 400 ms) pause overhead ≈ 3.5 s → ~16 s.
// Allow headroom for GT submit + slop.
...
ws.on('open', () => {
    console.log(`[smoke] connected, arming ${TRAJECTORY}`);
    send({
        type: 'experiment_arm',
        trajectory: TRAJECTORY,
        speed: SPEED,
        rep: 1,
        operatorNotes: 'yaw-roundtrip debug — from (0, 0) facing +X',
    });
});
    ...
    if (msg.type === 'experiment_completed') {
        ...
        if (msg.lastWaypointGt) {
            ...
            send({
                ...
                passFail: 'pass',
                notes: 'yaw-roundtrip auto-submit',
            });
        } else {
            console.log('[smoke] no camera waypoints — skipping auto-submit');
        }
        return;
    }
