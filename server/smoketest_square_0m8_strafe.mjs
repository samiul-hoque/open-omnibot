// Smoke-test runner for square_0m8_strafe.
//
...
//
// Inspect afterwards: evaluation/snapshots/square_0m8_strafe/<runId>/*.png +
// the run's meta.json under server/logs/.
import WebSocket from 'ws';

const URL = 'ws://localhost:3000/ws';
const TRAJECTORY = 'square_0m8_strafe';
const SPEED = 0.10;

// Motion budget: 8 × 0.4 m translates at 0.10 m/s (32 s) + 9 × (settle
// 300 ms + snapshot lead 400 ms) pause overhead (≈6.3 s) ≈ 38 s. Allow
// headroom for GT submit + slop.
...
ws.on('open', () => {
    ...
    send({
        ...
        speed: SPEED,
        rep: 1,
        operatorNotes: 'smoke test — square_0m8_strafe from (-0.4, -0.35) +Y',
    });
});
